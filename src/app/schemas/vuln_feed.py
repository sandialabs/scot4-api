...
class VulnFeedCreate(VulnFeedBase):
    owner: Annotated[str, Field(...)] = settings.FIRST_SUPERUSER_USERNAME


class VulnFeedUpdate(VulnFeedBase):
    owner: Annotated[str | None, Field(...)] = None


# pretty
class VulnFeed(PopularityVoted, FavoriteLink, VulnFeedBase, ResultBase):
...
class VulnFeedSearch(SearchBase):
