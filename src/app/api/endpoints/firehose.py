...
async def stream_audits(
    *,
    ...
    try:
        current_user = None  # this object will expire at end of session scope
    ...
    async def event_generator(audit_checkpoint, notification_checkpoint, current_user_id, current_user_username):
        try:
            while not shutdown:
                with SessionLocal() as db:
                    ...
                    for record in _audits:
                        if record[6] is not None and record[3] == 'link':
                            ...
                            if record[5] == current_user_username:
                    ...
                    notifications = db.query(Notification.id, Notification.created).filter(Notification.id > notification_checkpoint).filter(Notification.user_id == current_user_id).order_by(Notification.id.desc()).all()
                    if len(notifications) > 0:
                        notification_checkpoint = notifications[0][0]
                    for record in notifications:
                        yield json.dumps({'what': 'create', 'when': record[1].strftime('%Y-%m-%d %H:%M:%s'), 'element_type': 'notification', 'element_id': record[0], 'username': current_user_username})
                await asyncio.sleep(2)
        except asyncio.CancelledError as e:
            pass

    return EventSourceResponse(event_generator(audit_checkpoint, notification_checkpoint, current_user_id, current_user_username))
