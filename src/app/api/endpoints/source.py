...
def source_target_appearance(
    ...
    sources, _ = crud.source.query_with_filters(db, roles, filter_dict)
    if len(sources) == 0:
        raise HTTPException(404, "No Sources were found")

    try:
        _result, _count = crud.link.target_filter(db, [a.id for a in sources], TargetTypeEnum.source, target_types, and_, skip, limit, sort)
...
@router.get(
    ...
    summary="Get all target types for sources"
)
def source_target_appearance(
    id: Annotated[str, Query(...)] = None,
    name: Annotated[str, Query(...)] = None,
    ...
    """
    try:
        filter_dict, _ = get_appearance_filters(schemas.SourceSearch, id, name)
    except Exception as e:
        raise HTTPException(422, str(e))
    ...
    try:
        return crud.link.target_types(db, [a.id for a in sources], TargetTypeEnum.source)
    except Exception as e:
        raise HTTPException(422, str(e))
