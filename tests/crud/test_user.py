...
def test_create_in_object_user(db: Session, faker: Faker) -> None:
    email = faker.safe_email()
    user = UserCreate(
        username=email,
        email=email,
    ...
    db_obj = crud.user.create_in_object(db, obj_in=user, source_type=TargetTypeEnum.alertgroup, source_id=alert_group.id)
