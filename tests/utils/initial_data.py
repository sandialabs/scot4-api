...
def main() -> None:
    ...
    try:
        ...
        for _ in range(5):
            ...
            create_apikey(db_session, faker, user.username)
            # make user a handler?
            if faker.pybool():
        ...
        parent_entry = create_random_entry(db_session, faker)
        ...
        for _ in range(5):
            signature = create_random_signature(db_session, faker, random.choice(users).username)
            logger.info(f"Generating Random Entries for Signature {signature.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.signature, target_id=signature.id)
        ...
        for _ in range(5):
            alertgroup = create_random_alertgroup(
                db_session, signature_names_types, unique_signature_types, faker
            )
            ...
            logger.info(f"Generating Random Entries for Alert Group {alertgroup.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.alertgroup, target_id=alertgroup.id)
        ...
        for _ in range(5):
            event = create_random_event(
                db_session, faker, random.choice(users).username
            )
            ...
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.event, target_id=event.id)
        ...
        for _ in range(5):
            vuln_feed = create_random_vuln_feed(
                db_session, faker, random.choice(users).username
            )

            logger.info(f"Generating Random Entries for Vulnerability Queues {vuln_feed.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.vuln_feed, target_id=vuln_feed.id)
        ...
        for _ in range(5):
            track = create_random_vuln_track(
                db_session, faker, random.choice(users).username
            )

            logger.info(f"Generating Random Entries for Vulnerability Track {track.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.vuln_track, target_id=track.id)
        ...
        for _ in range(5):
            ...
            guide = create_random_guide(
                db_session, faker, random.choice(users).username, signature
            )
            logger.info(f"Generating Random Entries for Signature {signature.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.signature, target_id=signature.id)
        ...
        for _ in range(5):
            ...
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.incident, target_id=incident.id)
        ...
        for _ in range(5):
            dispatch = create_random_dispatch(
                db_session, faker, random.choice(users).username
            )
            ...
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.dispatch, target_id=dispatch.id)
        ...
        for _ in range(5):
            intel = create_random_intel(
                db_session, faker, random.choice(users).username
            )
            logger.info(f"Generating Random Entries for Intel {intel.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.intel, target_id=intel.id)
        ...
        for _ in range(5):
            ...
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.product, target_id=product.id)
        ...
        for _ in range(5):
            checklist = create_random_checklist(
                db_session, faker, random.choice(users).username
            )
            logger.info(f"Generating Random Entries for Checklist {checklist.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.checklist, target_id=checklist.id)
        ...
        for _ in range(5):
            feed = create_random_feed(db_session, faker, random.choice(users).username)
            logger.info(f"Generating Random Entries for Feed {feed.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.feed, target_id=feed.id)
        ...
        for _ in range(5):
            file = create_random_file(db_session, faker, random.choice(users).username)
            logger.info(f"Generating Random Entries for File {file.id}")
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.file, target_id=file.id)
        ...
        for _ in range(5):
            signature = random.choice(signatures)
            audit = create_audit(db_session, faker, signature.owner, signature)
        ...
        for _ in range(5):
            ...
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.threat_model_item, target_id=threat_model_item.id)
        ...
        for _ in range(5):
            ...
            create_random_entry(db_session, faker, parent_entry_id=parent_entry.id, target_type=TargetTypeEnum.stat, target_id=stat.id)
